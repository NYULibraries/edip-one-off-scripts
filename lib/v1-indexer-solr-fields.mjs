...
const componentSolrFields = {
    solrizer : {
        ...
        nonXpath  : {
            // This is the "author" field created in addition to and using
            // the data from `additional_component_fields()` -- same-named field
            // in `xpath`.
            author : {
                basename     : 'author',
                source       : '`author_teim` data, if it exists, which strangely' +
                               ' does not ever appear to be the case, because it\'s' +
                               ' not defined in the OM Terminology and not included' +
                               ' in `additional_component_fields()`.  Note that' +
                               ' if it did, it would simply overwrite the existing' +
                               ' `author_teim` field with the exact same data,' +
                               ' in addition to adding `author_sim` and `author_ssm`.',
                indexAsArray : [ SEARCHABLE, DISPLAYABLE, FACETABLE ],
            },
            // This is the "collection" field created in addition to and using
            // the data from `additional_component_fields()` -- same-named field
            // in `xpath`.
            collection : {
                basename     : 'collection',
                source       : '`collection_sim` data, if it exists.  Note that' +
                               ' in v1 indexer, this simply overwrites the existing' +
                               ' `collection_sim` field with the exact same data,' +
                               ' in addition to adding `collection_ssm` and ' +
                               ' `collection_teim`.',
                indexAsArray : [ SEARCHABLE, DISPLAYABLE, FACETABLE ],
            },
            // This is the "collection_unitid" field created in addition to and using
            // the data from `additional_component_fields()` -- same-named field
            // in `xpath`.
            collection_unitid : {
                basename     : 'collection_unitid',
                source       : '`collection_unitid_ssm` data, if it exists.  Note that' +
                               ' in v1 indexer, this simply overwrites the existing' +
                               ' `collection_unitid_ssm` field with the exact same data,' +
                               ' in addition to adding `collection_teim`.',

    ...
    solrizer    : {
        composite : {
            creator : {
                ...
                process      : 'Get elements for each xpath query in the order listed.' +
                               ...
                               ' and remove `nil` and duplicate values.' +
                               ' Note that these xpath queries are case-sensitive.' +
        ...
        xpath     : {
            ...
            chronlist          : {
                ...
                indexAsArray : [ SEARCHABLE ],
                process: 'Strip leading and trailing whitespace, where whitespace' +
                         ' is defined by https://ruby-doc.org/core-2.5.5/String.html#method-i-strip:' +
            ...
            unitdate_end       : {
                ...
                process      : 'For `unitdate_normal` fields, no extra processing' +
                               ' is done.  The processing for `unitdate_start` and' +
                               ' `unitdate_end`: for each element that matches' +
                               ' /(\\d{4})\\/(\\d{4})/, take the number on the' +
                               ' left of the slash for `unitdate_start` and the' +
                               ' number on the right of the slash for `unitdate_end.`' +
                               ' Remove all duplicates.  For `*_si` fields, only' +
                               ' use the value from the last element.',
            ...
            unittitle          : {
                xpath        : 'archdesc[@level=\'collection\']/did/unittitle',
                process      : 'For `unittitle_teim`, and all proxy fields based ' +
                               ' on `unittitle` such as `collection` and `heading` ' +
...
const componentSolrFields = {
    solrizer : {
        composite : {
            ...
            parent_unittitles : {
                ...
                process      : 'For each ancestor <c>, starting from the earliest' +
                               ' ancestor to the immediate parent, extract' +
                               ' the text of the first <unititle> element (all' +
                               ' tags stripped), sanitize (see https://jira.nyu.edu/browse/DLFA-209),' +
                               ' and add to array.  If there are no <unittitle>' +
                               ' elements, collect the first <unitdate> element' +
                               ' instead, using the same processing instructions.' +
                               ' If there are no <unitdate> elements, add "[No title available]"' +
                               ' to array.',
        ...
        nonXpath  : {
            author             : {
                basename: 'author',
                source: 'Value of `author_teim` from the collection doc',
                process: 'ERROR: This appears to never gets populated due to a probable bug: https://jira.nyu.edu/browse/DLFA-211?focusedCommentId=8577864&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-8577864',
                indexAsArray : [ SEARCHABLE, DISPLAYABLE, FACETABLE ],
            },
            componentChildren                      : {
            ...
            sort             : {
                basename     : 'sort',
                source       : 'Loop counter for array of <c> elements in occurrence order',
                process      : 'Start at 1 and increment by 1 for every <c> in the EAD file, with the <c> elements incremented in file occurrence order.',
                dataType     : DATA_TYPES.INTEGER,
                indexAsArray : [ SORTABLE ],
            },
            unitdateDisplay                        : {
        ...
        xpath     : {
            ...
            chronlist : {
                ...
                process      : 'Remove all empty string and `nil` elements from' +
                               ' the array, and strip leading and trailing whitespace,' +
                               ' where whitespace is defined by' +
                               ' https://ruby-doc.org/core-2.5.5/String.html#method-i-strip:' +
                               ' null, horizontal tab, line feed, vertical tab, form feed, carriage return, space',
                indexAsArray : [ SEARCHABLE ],
            },
